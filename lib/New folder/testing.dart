...
//     try {
//       final response = await fetchSchedulerSettings(widget.erpUrlBase).timeout(
//         const Duration(seconds: 10),
//         onTimeout: () =>
//             throw TimeoutException('Scheduler settings fetch timed out'),
//       );
//       setState(() {
...
//         userOptions = List<String>.from((response['operators'] as List<dynamic>)
//             .map((operator) => operator['name'] as String));
//         operators = List<Map<String, dynamic>>.from(response['operators']);
...
//     // Initialize with saved elapsed seconds or accumulated duration
//     int savedSeconds = elapsedSeconds[processName] ?? 0;
//     if (savedSeconds == 0 && startDateTime != null) {
...
//         .where((name) => name.startsWith('SP'))
//         .map((name) => int.tryParse(name.replaceFirst('SP', '')) ?? 0)
//         .toList();
//     final maxSpNumber =
...
//             ? DateTime.tryParse(process['date_start'].toString())
//             : null;
//         setState(() {
//           elapsedSeconds[processName] = savedSeconds;
//           process['duration'] = _formatTime(savedSeconds);
//           if (status == 'progress' && startDateTime != null) {
//             _startTimer(processName, startDateTime, savedSeconds);
//           }
//         });
//       }
//     } catch (e) {
...
//           {
//             'id': '',
//             'name': 'SP1', // Changed to uppercase SP1
//             'qty': int.tryParse(
...
//           selectedAssemblyTypes[processName] =
//               selectedAssemblyTypes[processName] ?? null;
//           selectedDefects[processName] = selectedDefects[processName] ?? null;
//         });
//       });
//     } finally {
//       setState(() {
...
//                     selectedDefects[processName] ?? null;
//               });
//             }
//             return;
//           }
...
//                   assemblyTypes.contains(machineName)
//                       ? machineName
//                       : (selectedAssemblyTypes[targetProcessName] ?? null);
//               selectedDefects[targetProcessName] =
//                   defectTypes.contains(defectReason)
//                       ? defectReason
//                       : (selectedDefects[targetProcessName] ?? null);
//             }
//           } else {
//             processes = [];
//             for (var record in records) {
//               final status =
//                   record['status'] == 'resume' ? 'progress' : record['status'];
//               final processName = record['name'] ?? 'SP1';
//               final machineName = record['machine_name'] as String?;
//               final defectReason = record['defect_reason'] as String?;
//               final accumulatedDuration = _parseDuration(record['duration']);
//               final savedSeconds = elapsedSeconds[processName] ?? 0;
...
//                                                     BorderRadius.zero),
//                                           ),
//                                           child: const Text('Hold', // Changed from Pause to Hold
//                                               style: TextStyle(
//                                                   color: Colors.white)),
//                                         )
