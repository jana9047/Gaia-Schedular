...
class _SubmitProcessPageState extends State<SubmitProcessPage> {
  ...
  Future<int> _loadElapsedSeconds(String processName) async {
    final prefs = await SharedPreferences.getInstance();
    print('Loading elapsedSeconds for $processName');
    print('Key: elapsedSeconds_${widget.processData['id']}_$processName');
    print(prefs
        .getInt('elapsedSeconds_${widget.processData['id']}_$processName'));
    return prefs.getInt(
            'elapsedSeconds_${widget.processData['id']}_$processName') ??
        0;
  }
  ...
  void _startTimer(
    ...
    // Set the elapsed seconds to the accumulated duration
    elapsedSeconds[processName] = accumulatedDuration;

    print(
        'Starting timer for $processName with accumulated duration: $accumulatedDuration');
  ...
  Future<void> _saveProcessStartTime(
      String processName, DateTime startTime) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      'processStartTime_${widget.processData['id']}_$processName',
      startTime.toIso8601String(),
    );
    print('Saved start time for $processName: ${startTime.toIso8601String()}');
  }

// Method to load stored start/resume datetime
  Future<DateTime?> _loadProcessStartTime(String processName) async {
    final prefs = await SharedPreferences.getInstance();
    final startTimeString = prefs
        .getString('processStartTime_${widget.processData['id']}_$processName');
    if (startTimeString != null) {
      try {
        final startTime = DateTime.parse(startTimeString);
        print(
            'Loaded start time for $processName: ${startTime.toIso8601String()}');
        return startTime;
      } catch (e) {
        print('Error parsing stored start time for $processName: $e');
        return null;
      }
    }
    return null;
...
// Method to clear stored start time (when process is paused/stopped)
  Future<void> _clearProcessStartTime(String processName) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs
        .remove('processStartTime_${widget.processData['id']}_$processName');
    print('Cleared stored start time for $processName');
  }

  void _startProcess(String processName) async {
    flag = 1;

    // Store current datetime when starting
    final currentTime = DateTime.now();
    await _saveProcessStartTime(processName, currentTime);

    setState(() {
      elapsedSeconds[processName] = 0;
      _saveElapsedSeconds(processName, 0); // Save 0 to SharedPreferences
    });
    print('Starting process $processName at $currentTime');
  ...
  void _pauseProcess(String processName) async {
    ...
    // Clear stored start time when pausing
    await _clearProcessStartTime(processName);
  ...
  void _resumeProcess(String processName) async {
    ...
    // Store current datetime when resuming
    final currentTime = DateTime.now();
    await _saveProcessStartTime(processName, currentTime);

    print('Resuming process $processName at $currentTime');

    // Don't modify elapsedSeconds here - let _fetchUpdatedProcessStatus handle it
  ...
  void _stopProcess(String processName) async {
    ...
    // Clear stored start time when stopping
    await _clearProcessStartTime(processName);
  ...
  Future<void> _fetchInitialProcessStatus() async {
    ...
    try {
      ...
      for (var process in processes) {
        ...
        print('saved seconds for $processName: $savedSeconds');
        if (savedSeconds > 0 && process['status'] != 'progress') {
          setState(() {
            ...
            print(
                'Setting initial duration for $processName: ${process['duration']}');
  ...
  Future<void> _fetchUpdatedProcessStatus([String? targetProcessName]) async {
    ...
    try {
      ...
      if (widget.erpUrlBase == "https://www.alfadock-pack.com") {
          ...
          if (record.isNotEmpty) {
            ...
            if (sid > 0) {
              ...
              if (lastStartTime != null) {
                // Parse lastStartTime to DateTime
                ...
                newDurationSeconds =
                    currentTime.difference(lastStart).inSeconds;
                print(
        ...
        // NON-ALFADOCK URLS WITH FIXED DATETIME STORAGE FUNCTIONALITY
        ...
        if (targetProcessName != null) {
          // Handle specific process update
          final record = records.firstWhere(
            (r) =>
          ...
          if (record.isNotEmpty) {
            await _updateSingleProcess(record, targetProcessName, true);
          }
        } else {
          ...
          for (var record in records) {
            ...
            await _updateSingleProcess(record, processName, false);
          }
          setState(() {});
        }
      }
  ...
  Future<void> _updateSingleProcess(Map<String, dynamic> record,
      String processName, bool isTargetUpdate) async {
    print('record123: ${jsonEncode(record)}');

    final status = record['status'] == 'resume' ? 'progress' : record['status'];
    ...
    final defectCodeId = defectCodeIdStr != null && defectCodeIdStr.isNotEmpty
    ...
    // CRITICAL: Load the most current savedSeconds from SharedPreferences
    final savedSeconds = await _loadElapsedSeconds(processName);
    print('Loaded savedSeconds for $processName: $savedSeconds');

    final updatedProcess = {
      'id': record['id']?.toString() ??
          (isTargetUpdate
              ? processes.firstWhere((p) => p['name'] == processName,
                  orElse: () => {'id': ''})['id']
              : ''),
      'name': processName,
      'qty': record['qty'] ??
      ...
      'defect_qty': record['defect_qty'] ??
          (isTargetUpdate
              ? processes.firstWhere((p) => p['name'] == processName,
                  orElse: () => {'defect_qty': 0})['defect_qty']
              : 0),
      'status': status,
      'duration': status == 'ready'
          ? '00:00:00'
          : _formatTime(savedSeconds > 0 ? savedSeconds : accumulatedDuration),
      'date_start': record['initial_date_start_UTC'] ??
          (isTargetUpdate
              ? processes.firstWhere((p) => p['name'] == processName,
                  orElse: () => {'date_start': null})['date_start']
              : null),
      'date_end': status == 'ready'
          ? null
          : (record['date_end_UTC'] ??
              (isTargetUpdate
                  ? processes.firstWhere((p) => p['name'] == processName,
                      orElse: () => {'date_end': null})['date_end']
                  : null)),
      'users2': (record['users2'] as List<dynamic>?)?.map((user) {
            return operators.firstWhere(
              (op) => op['id'] == user['id'] && op['name'] == user['name'],
              orElse: () => {
                'id': user['id'],
                'name': user['name'],
                'alfadock_id': user['id'] ?? 0,
    ...
    // UNIFIED TIMER LOGIC - Use this for both target updates and full refreshes
    await _handleProcessTimer(processName, currentStatus, startDateTime,
        savedSeconds, accumulatedDuration);
...
// New unified timer handling method
  Future<void> _handleProcessTimer(
      String processName,
      String currentStatus,
      DateTime? startDateTime,
      int savedSeconds,
      int accumulatedDuration) async {
    if (currentStatus == 'progress') {
      // Check if we have a stored start time for this process
      final storedStartTime = await _loadProcessStartTime(processName);

      if (storedStartTime != null) {
        // Calculate elapsed time from stored start time to now
        final now = DateTime.now();
        final timeDifference = now.difference(storedStartTime).inSeconds;

        // Use savedSeconds as base (this contains the accumulated time before the current session)
        final adjustedSeconds = savedSeconds + timeDifference;

        print(
            'Adjusting timer for $processName: saved=$savedSeconds, time diff=$timeDifference, total=$adjustedSeconds');

        // Update elapsedSeconds immediately to maintain consistency
        elapsedSeconds[processName] = adjustedSeconds;
        ...
        // No stored start time, use savedSeconds or accumulated duration
        final initialSeconds =
            savedSeconds > 0 ? savedSeconds : accumulatedDuration;
        elapsedSeconds[processName] = initialSeconds;

        print(
            'Starting timer for $processName with initial seconds: $initialSeconds');
      ...
      // Clear stored start time when not in progress
      if (currentStatus == 'done' || currentStatus == 'pause') {
        await _clearProcessStartTime(processName);
      }
    }
  }
  ...
  Future<void> _uploadImage(File file) async {
    ...
    final today = DateTime.now(); // e.g., 02:00 PM IST, August 10, 2025
    final fileName =
        '${DateFormat('ddMMyyyyHHmmss').format(today)}.jpg'; // e.g., 10082025140000.jpg
    final fileLength = await file.length();
    print('Uploading file: $fileName, Length: $fileLength bytes');
    ...
    request.files.add(await http.MultipartFile.fromPath(
      'file',
      file.path,
      filename: 'camera.jpeg', // Match Swift's filename
    ));
    ...
    // Set form fields to match Swift code exactly
    request.fields['userid'] = widget.userId ?? '0';
    request.fields['filename'] = fileName; // Use lowercase 'filename'
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Parse response as plain string (not JSON), matching Swift behavior
        if (responseData.isNotEmpty) {
          // Optionally, associate file with process (equivalent to setProcessForFile)
          print(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Photo uploaded successfully: $fileName')),
          );
...
// Equivalent to Swift's setProcessForFile
  Future<void> _setProcessForFile(String fileIdResponse, int processId) async {
    try {
      ...
      // Optionally, handle this error gracefully (e.g., log it but don't fail the upload)
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Replace your current IconButton with this:
          IconButton(
            icon: isLoading
                ? const SizedBox(
                    width: 20,
            ...
            onPressed: isLoading ? null : _openCamera,
          ),
              ...
              Column(
                ...
                children: [
                  ...
                  if (fileList.isNotEmpty || folderStack.isNotEmpty)
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final String? thumbnailGuid =
                                    file['thumbnailGuid'];
                                print('thumbnailGuid sdads: $thumbnailGuid');
                                final bool isFolder = file['fileType'] == 1;
                                final String fileName =
                                    file['fileName'] ?? 'Unnamed';
                                final String? guid = file['guid'];
                                final String? folderId = file['id']?.toString();
                    ...
                    Text('No files or folders available'),
                ],
              ),
              Expanded(
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final List<String> currentSelectionNameserp =
                            ...
                            return _userDisplay == 1
                                ? (mappedValue.isNotEmpty ? mappedValue : key)
                                : key;
                          }).toList();

                          // print(
                          //     'currentSelectionNameserp: $currentSelectionNameserp');
                          // print('janasas ${selectedUsers[processName]}');
                          final quantity =
                          ...
                          final startDateTime = (process['status']
                                      ?.toString() ==
                                  'ready')
                              ? null
                                  ...
                                  Wrap(
                                    ...
                                    children: (selectedUsers[processName] ?? [])
                                        ...
                                        : (processes[index]['users2']
                                            ...
                                            // print(
                                            //     'display $currentSelectionNameserp');
                                            // print('user: $user');
                                            final displayName =
                                                (_userDisplay == 1
