...
// Add this method to calculate the correct elapsed time from API response
  int _calculateElapsedTimeFromAPI(Map<String, dynamic> record) {
    try {
      ...
      // Get date_start_UTC and convert to local time
      final dateStartUTCStr = record['date_start_UTC']?.toString();
      if (dateStartUTCStr != null && dateStartUTCStr.isNotEmpty) {
        try {
          // Parse the UTC time
          final utcDateTime = DateTime.parse(dateStartUTCStr).toUtc();

          // Get current time in UTC
          final currentUTC = DateTime.now().toUtc();

          // Calculate elapsed seconds since date_start_UTC
          final elapsedSinceStart =
              currentUTC.difference(utcDateTime).inSeconds;
        ...
        if (targetProcessName != null) {
          ...
          for (var record in records) {
            ...
            if (currentStatus == 'progress' || currentStatus == 'resume') {
              // Calculate correct elapsed time from API
              ...
              // For non-progress states, still calculate the elapsed time for display
              final calculatedElapsed = _calculateElapsedTimeFromAPI(record);
