          ...
          for (var record in records) {
            ...
            if (currentStatus == 'progress') {
              ...
              final storedStartTime =
                  await _loadProcessStartTime(targetProcessName ?? processName);

              if (storedStartTime != null) {
                // Calculate elapsed time from stored start time to now
                final now = DateTime.now();
                final timeDifference =
                    now.difference(storedStartTime).inSeconds;

                // Add the time difference to saved seconds
                final savedSeconds =
                    elapsedSeconds[targetProcessName ?? processName] ?? 0;
                final adjustedSeconds = savedSeconds + timeDifference;

                print(
                    'Adjusting timer for ${targetProcessName ?? processName}: saved=$savedSeconds, time diff=$timeDifference, total=$adjustedSeconds');
                ...
                // No stored start time, use original logic
                _startTimer(
                  targetProcessName ?? processName,
                  startDateTime ??
                      (updatedProcess['date_start'] != null
              ...
              _stopTimer(targetProcessName ?? processName);
              if (status == 'done' || status == 'pause') {
                await _saveElapsedSeconds(targetProcessName ?? processName,
                    elapsedSeconds[targetProcessName ?? processName] ?? 0);
                // Clear stored start time when not in progress
                await _clearProcessStartTime(targetProcessName ?? processName);
              }
            }
            final apiUsers = (record['users2'] as List<dynamic>?)
                    ?.map((user) => user['name'] as String)
